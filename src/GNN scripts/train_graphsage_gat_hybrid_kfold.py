"""
Final k-fold training for GraphSAGE-GAT Hybrid with best hyperparameters.
...
from models import GraphSAGEGATHybrid
from utils import set_seed, get_device, count_parameters
from metrics import compute_metrics_dict
from split_asserts import assert_split_integrity


class GraphSAGEGATHybridKFoldTrainer:
    """K-fold trainer for GraphSAGE-GAT Hybrid with best hyperparameters."""
    ...
    def __init__(
        ...
        self.run_dir = os.path.join(out_dir, f"graphsage_gat_hybrid_{timestamp}")
        ...
        print(f"\n{'='*80}")
        print(f"K-Fold Training: GraphSAGE-GAT Hybrid")
        print(f"{'='*80}")
    ...
    def _create_model(self):
        """Create GraphSAGE-GAT Hybrid model instance."""
    ...
    def _save_summary(self, all_fold_results):
        ...
        summary = {
            'model_name': 'graphsage_gat_hybrid',
            'hyperparams': self.hyperparams,
...
def main():
    parser = argparse.ArgumentParser(description="K-fold training for GraphSAGE-GAT Hybrid with best hyperparameters")
    ...
    # Train
    trainer = GraphSAGEGATHybridKFoldTrainer(
        pkl_path=args.pkl,

#!/usr/bin/env python3
"""
Ablation Study for Hybrid GCN+GraphSAGE Model (REVAMP Version)

Adapted from original ablation study for:
- HybridGCNGraphSAGE model
- Single fixed split
- Validation AUROC selection
- Enhanced persistence with top 20 predictions
"""
...
from split_manager import load_single_split
...
def load_concept_names(concept_csv_path: str, node_ids: List[str]) -> Dict[str, str]:
    ...
    Args:
        concept_csv_path: Path to concept.csv file
        node_ids: List of concept IDs (as strings)
...
def apply_feature_ablation(X: torch.Tensor, is_obstetric: np.ndarray, ablation_type: str) -> torch.Tensor:
    ...
    Feature indices (9 total):
    0: degree
    1: clustering_coeff
    2: betweenness
    3: closeness
    4: pagerank
    5: in_obstetric_community
    6: log_degree
    7: eigenvector_centrality
    8: katz_centrality
    
    Args:
        X: Feature matrix (num_nodes, 9)
        is_obstetric: Boolean array indicating obstetric nodes
        ablation_type: Type of feature ablation
    ...
    if ablation_type == "none":
        ...
        X_ablated = X.clone()
        X_ablated[:, 5] = 0  # Zero out in_obstetric_community
        print(f"  Zeroed out 'in_obstetric_community' feature (index 5)")
        
    elif ablation_type == "local_only":
        indices = [0, 6, 1]  # degree, log_degree, clustering_coeff
        X_ablated = X[:, indices]
...
def apply_graph_structure_ablation(
    ...
    Args:
        edge_index_mp: Leakage-safe message passing graph (from split, with val/test removed)
        edge_index_full: Full graph edge index
        is_obstetric: Boolean array indicating obstetric nodes
        ablation_type: Type of structure ablation
        val_pos: Validation positive edges (to exclude from rewiring)
        test_pos: Test positive edges (to exclude from rewiring)
        seed: Random seed for rewiring
        
    Returns:
        Modified edge index for message passing
    """
    ...
    if ablation_type in ["none", "no_community_flag", "local_only", "global_only", "degree_only"]:
        # For feature ablations, use the pre-computed leakage-safe edge_index_mp
        ...
        # Keep only cross-community edges from the leakage-safe MP graph
        mask = []
        ...
        # Keep only within-community edges from the leakage-safe MP graph
        mask = []
        for i in range(edge_index_mp.shape[1]):
            u, v = edge_index_mp[0, i].item(), edge_index_mp[1, i].item()
        ...
        # Degree-preserving rewiring on the leakage-safe MP graph
        print(f"  Rewiring graph (degree-preserving)...")
        rng = np.random.RandomState(seed)
        
        # Create forbidden edge set (val + test edges that must NOT be created)
        forbidden_edges = set()
        for edge in val_pos:
        ...
        for _ in range(num_swaps):
            ...
            if u != y and x != v:
                ...
                # CRITICAL: Ensure new edges are not in forbidden set and not already in graph
        ...
        return edge_index_ablated
        edge_index_ablated = torch.tensor(edge_array.T, dtype=torch.long)
        print(f"  Rewired: {successful_swaps} successful swaps, {edge_index_ablated.shape[1]} edges")
        return edge_index_ablated
...
class AblationTrainer:
    """Trainer for single ablation configuration."""
    ...
    def __init__(
        ...
        if node_ids is not None and os.path.exists(concept_csv_path):
            ...
            # Create list indexed by node index
            self.node_names = [concept_map.get(node_ids[i], node_ids[i]) for i in range(len(node_ids))]
        else:
            self.node_names = None
        
        # Load split
        print(f"Loading split from {splits_path}...")
        split = load_single_split(splits_path)
        
        # Store split data before ablation
        self.val_pos = split['val_pos']
        self.val_neg = split['val_neg']
        self.test_pos = split['test_pos']
        self.test_neg = split['test_neg']
        
        # Apply feature ablation
        self.x = apply_feature_ablation(self.x, self.is_obstetric, ablation_config['ablation_type'])
        
        # Apply graph structure ablation
        # CRITICAL: Start with leakage-safe edge_index_mp from split (has val/test removed)
        edge_index_mp_safe = split['edge_index_mp']
        self.edge_index_mp = apply_graph_structure_ablation(
            edge_index_mp_safe,
            self.edge_index, 
            self.is_obstetric,
            ablation_config['ablation_type'],
            self.val_pos,  # Pass val edges to prevent leakage in rewiring
            self.test_pos,  # Pass test edges to prevent leakage in rewiring
            seed
        )
        
        # Apply training data subsampling
        train_pos_orig = split['train_pos']
        train_neg_orig = split['train_neg']
        
        self.train_pos, self.train_neg = subsample_training_data(
            train_pos_orig,
            train_neg_orig,
            ablation_config['train_fraction'],
            seed
        )
        ...
        self.edge_index_mp = self.edge_index_mp.to(self.device)
        
        # CRITICAL: Assert split integrity before training
        print("\n" + "="*80)
        print("RUNNING SPLIT INTEGRITY CHECKS")
        print("="*80)
        assert_split_integrity(
            ...
            self.edge_index_mp,
            self.edge_index,
            fold_id=None,  # Single split, not k-fold
        ...
        print(f"\nDataset sizes:")
        print(f"  Train: {len(self.train_pos)} pos, {len(self.train_neg)} neg")
        print(f"  Val: {len(self.val_pos)} pos, {len(self.val_neg)} neg")
        print(f"  Test: {len(self.test_pos)} pos, {len(self.test_neg)} neg")
    
    def train(self):
        """Train model with early stopping on validation AUROC."""
        set_seed(self.seed)
        ...
        train_pos_t = torch.tensor(self.train_pos, dtype=torch.long).t().to(self.device)
        train_neg_t = torch.tensor(self.train_neg, dtype=torch.long).t().to(self.device)
        val_pos_t = torch.tensor(self.val_pos, dtype=torch.long).t().to(self.device)
        val_neg_t = torch.tensor(self.val_neg, dtype=torch.long).t().to(self.device)
        test_pos_t = torch.tensor(self.test_pos, dtype=torch.long).t().to(self.device)
        test_neg_t = torch.tensor(self.test_neg, dtype=torch.long).t().to(self.device)
        ...
        for epoch in range(self.max_epochs):
            ...
            z = model(self.x, self.edge_index_mp)
            ...
            # Check improvement (VALIDATION AUROC)
            ...
            if val_auroc > best_val_auroc:
                ...
                checkpoint_path = os.path.join(self.run_dir, 'best_checkpoint.pt')
                torch.save({
                    'epoch': epoch,
                ...
                self._save_predictions(
                    'train', train_edge_index, train_labels, train_probs
                )
                self._save_predictions(
                    'val', val_edge_index, val_labels, val_probs
                )
                self._save_predictions(
                    'test', test_edge_index, test_labels, test_probs
                )
                
                # Save top 20 predictions
                top20 = {
                    'train': get_top_predictions(train_edge_index.cpu(), train_probs, train_labels.cpu().numpy(), self.node_names),
                    'val': get_top_predictions(val_edge_index.cpu(), val_probs, val_labels.cpu().numpy(), self.node_names),
                    'test': get_top_predictions(test_edge_index.cpu(), test_probs, test_labels.cpu().numpy(), self.node_names)
                }
                
                with open(os.path.join(self.run_dir, 'top_20_predictions.json'), 'w') as f:
        ...
        summary = {
            ...
            'train_metrics': {k.replace('train_', ''): v for k, v in best_metrics.items() if k.startswith('train_')},
            'val_metrics': {k.replace('val_', ''): v for k, v in best_metrics.items() if k.startswith('val_')},
            'test_metrics': {k.replace('test_', ''): v for k, v in best_metrics.items() if k.startswith('test_')}
        }
        ...
        print(f"Test AUROC: {best_metrics['test_auroc']:.4f}")
        print(f"Test AUPRC: {best_metrics['test_auprc']:.4f}")
        print(f"\nResults saved to {self.run_dir}")
        
        return summary
    
    def _save_predictions(self, split_name: str, edge_index: torch.Tensor, 
                         labels: torch.Tensor, probs: np.ndarray):
        """Save predictions for a split."""
        save_path = os.path.join(self.run_dir, f'{split_name}_predictions.npz')
        
        # Get pos/neg edges
        if split_name == 'train':
            pos_edges, neg_edges = self.train_pos, self.train_neg
        elif split_name == 'val':
            pos_edges, neg_edges = self.val_pos, self.val_neg
        else:
            pos_edges, neg_edges = self.test_pos, self.test_neg
...
def main():
    parser = argparse.ArgumentParser(description="Hybrid GCN+GraphSAGE Ablation Study (REVAMP)")
    ...
    parser.add_argument('--splits', type=str, default='splits/single_seed42.npz',
                       help='Path to single split file')
    ...
    parser.add_argument('--out_dir', type=str, default='ABLATIONS/runs')
